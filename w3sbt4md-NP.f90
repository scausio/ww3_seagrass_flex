...
! 0.  Initializations ------------------------------------------------ *
      IF ( FIRST ) THEN
          CALL INSBT4
!
!Read the seagrass mask in the ww3 grid
!
          Read(unit) maskseagrass
!
! extract the vegetation parameters
!
! first extract 0,1 values (no, with seagrass)
!
	do
        seagrass()=maskseagrass(., 1)
        enddo
!
! read the alpha values
!
        do
        alpha()=maskseagrass(., 2)
        enddo
...
! Ivan: 3 is the number of vertex of the basic grid triangle
...
!!!!!!! nadia: why is the bottom friction only active when wn*dsub less than 6 meters?
...
!!
!!
          S(IS)=D(IS)*A(IS) + Seagrass(is)*Sveg(is)
!!!
          END DO
        END DO 
!
